...
export default function PromptExpander({ initialIdea = '' }: PromptExpanderProps) {
  ...
  const expandPrompt = () => {
    ...
    setTimeout(() => {
      const expanded: ExpandedPrompt = {
        role: `You are an expert advisor and strategist with years of practical experience helping people achieve tangible results.`,
        objective: `Your task is to help me ${idea.trim()}. Provide actionable, specific guidance that I can implement immediately.`,
        context: `I'm looking for practical advice that works in real-world scenarios. I want clear steps, not generic theory.`,
...
- Assume I'm committed but need clear direction`,
        followUpStrategy: `After providing your initial response, ask me 3-5 clarifying questions to refine the strategy based on my specific situation, experience level, and constraints.`
      };
      ...
      {expandedPrompt && (
        <div className="bg-white rounded-3xl shadow-md p-8 border border-gray-200">
          <div className="flex justify-between items-start mb-6">
            <h3 className="text-2xl font-bold text-gray-900">Your Pro Prompt</h3>
            <button
          ...
          <div className="bg-gray-50 rounded-2xl p-6 space-y-5 text-sm border border-gray-200">
            <div>
              <p className="font-bold text-gray-900 mb-2 text-sm">ROLE:</p>
              <p className="text-gray-700 leading-relaxed">{expandedPrompt.role}</p>
            </div>
            <div>
              <p className="font-bold text-gray-900 mb-2 text-sm">OBJECTIVE:</p>
              <p className="text-gray-700 leading-relaxed">{expandedPrompt.objective}</p>
            </div>
            <div>
              <p className="font-bold text-gray-900 mb-2 text-sm">CONTEXT:</p>
              <p className="text-gray-700 leading-relaxed">{expandedPrompt.context}</p>
            </div>
            <div>
              <p className="font-bold text-gray-900 mb-2 text-sm">CONSTRAINTS:</p>
              <p className="text-gray-700 whitespace-pre-line leading-relaxed">{expandedPrompt.constraints}</p>
            </div>
            <div>
              <p className="font-bold text-gray-900 mb-2 text-sm">OUTPUT FORMAT:</p>
              <p className="text-gray-700 whitespace-pre-line leading-relaxed">{expandedPrompt.outputFormat}</p>
            </div>
            <div>
              <p className="font-bold text-gray-900 mb-2 text-sm">ASSUMPTIONS:</p>
              <p className="text-gray-700 whitespace-pre-line leading-relaxed">{expandedPrompt.assumptions}</p>
            </div>
            <div>
              <p className="font-bold text-gray-900 mb-2 text-sm">FOLLOW-UP STRATEGY:</p>
              <p className="text-gray-700 leading-relaxed">{expandedPrompt.followUpStrategy}</p>
            </div>
          </div>

          <div className="mt-6 bg-amber-50 rounded-2xl p-5 border border-amber-200">
            <p className="text-sm text-gray-800">
              <span className="font-bold">Next Step:</span> Copy this prompt and paste it into ChatGPT, Claude, or your preferred AI tool for best results.
            </p>
          </div>
        </div>
      )}
